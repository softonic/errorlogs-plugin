 ...
 * @author Ulli Hafner
 */
public class ErrorLogsParser extends AbstractAnnotationParser {
    /** Unique identifier of this class. */
    private static final long serialVersionUID = -7705621875291182458L;

    /**
     * Creates a new instance of {@link ErrorLogsParser}.
     */
    public ErrorLogsParser() {
     ...
     */
    public ErrorLogsParser(final String defaultEncoding) {
    ...
    public Collection<FileAnnotation> parse(final InputStream file, final String moduleName) throws InvocationTargetException {
        try {
            ...
            digester.setClassLoader(ErrorLogsParser.class.getClassLoader());

            String rootXPath = "errorlog";
            digester.addObjectCreate(rootXPath, ErrorLogs.class);
            digester.addSetProperties(rootXPath);

            String fileXPath = "errorlog/file";
            digester.addObjectCreate(fileXPath, hudson.plugins.errorlog.parser.File.class);
            digester.addSetProperties(fileXPath);
            digester.addSetNext(fileXPath, "addFile", hudson.plugins.errorlog.parser.File.class.getName());
            ...
            ErrorLogs module;
            module = (ErrorLogs)digester.parse(new InputStreamReader(file, "UTF-8"));
            if (module == null) {
                throw new SAXException("Input stream is not a errorlog file.");
            }
     ...
     */
    private Collection<FileAnnotation> convert(final ErrorLogs collection, final String moduleName) {
        ArrayList<FileAnnotation> annotations = new ArrayList<FileAnnotation>();

        for (hudson.plugins.errorlog.parser.File file : collection.getFiles()) {
     ...
     */
    private boolean isValidWarning(final hudson.plugins.errorlog.parser.File file) {
