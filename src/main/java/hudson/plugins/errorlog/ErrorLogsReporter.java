 ...
 * @author Ulli Hafner
 */
public class ErrorLogsReporter extends HealthAwareReporter<ErrorLogsResult> {
    ...
    /**
     * Creates a new instance of <code>ErrorLogsReporter</code>.
     *
     * @param healthy
    ...
    public ErrorLogsReporter(final String healthy, final String unHealthy, final String thresholdLimit, final boolean useDeltaValues,
    ...
    public ParserResult perform(final MavenBuildProxy build, final MavenProject pom,
        ...
        FilesParser checkstyleCollector = new FilesParser(PLUGIN_NAME,
                CHECKSTYLE_XML_FILE, new ErrorLogsParser(getDefaultEncoding()), getModuleName(pom));

        return getTargetPath(pom).act(checkstyleCollector);
    }

    @Override
    protected ErrorLogsResult createResult(final MavenBuild build, final ParserResult project) {
        return new ErrorLogsReporterResult(build, getDefaultEncoding(), project);
    }

    @Override
    protected MavenAggregatedReport createMavenAggregatedReport(final MavenBuild build, final ErrorLogsResult result) {
        return new ErrorLogsMavenResultAction(build, this, getDefaultEncoding(), result);
    }

    @Override
    public List<ErrorLogsProjectAction> getProjectActions(final MavenModule module) {
        return Collections.singletonList(new ErrorLogsProjectAction(module, getResultActionClass()));
    }

    @Override
    protected Class<ErrorLogsMavenResultAction> getResultActionClass() {
