 ...
 * @author Ulli Hafner
 */
public final class ErrorLogsRules {
    /** Mapping of rule names to rules. */
    private final Map<String, Rule> rulesByName = new HashMap<String, Rule>();
    /** Singleton instance. */
    private static final ErrorLogsRules INSTANCE = new ErrorLogsRules();
     ...
     */
    public static ErrorLogsRules getInstance() {
        return INSTANCE;
    }
    ...
    public void initialize() {
        try {
            ...
            for (int i = 0; i < ruleFiles.length; i++) {
                InputStream inputStream = ErrorLogsRules.class.getResourceAsStream("config_" + ruleFiles[i] + ".xml");
    ...
    private void log(final Throwable exception) {
        Logger.getLogger(ErrorLogsRules.class.getName()).log(Level.SEVERE, "Can't initialize checkstyle rules.", exception);
    }
    ...
    private Digester createDigester() throws ParserConfigurationException {
        ...
        digester.setClassLoader(ErrorLogsRules.class.getClassLoader());
